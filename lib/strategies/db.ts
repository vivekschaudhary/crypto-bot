...
export async function upsertSingletonBotSession(
  ...
  await sql.begin(async (tx) => {
    ...
    // session (latest by started_at) so a save UPDATEs the active row, not a
    // stale ended (`reset`) one. Matches loadSingletonSession's selection.
    const existing = await tx<{ id: string }[]>`
      SELECT id FROM bot_sessions ORDER BY started_at DESC LIMIT 1 FOR UPDATE
    `;
