...
- [2026-06-17] [Engineer/Architect] **Built (confirms the above).** Real-money helpers in a NEW `lib/bot/manual-orders.ts` (`forceBuy`/`sellFraction`) — kept OUT of the safe `lib/bot/overrides.ts` so the invariant test could keep a STRUCTURAL guard that `overrides.ts` (pause/resume/reset) never reaches `lib/coinbase/orders`, while flipping the `/api/bot/**` graph assertion to MAY-reach. Route un-defers `force_buy`/`sell_50`/`sell_all` (dispatch w/ `asset`; un-validated → 400 missing/invalid-asset; reasons → 409 cap-reached/no-position, 502 price/placement). `aggregateSessionTotals` now `source IN ('bot','manual')` (combined cap). `ManualOverridesCard` = pure `ManualOverridesView` (render-tested across states) + container; mode-aware confirm; paper-mode line while dark. Behavioral tests: dry_run while dark (placeOrder NEVER called) / live places / cap+no-position+invalid-asset refuse without placing. Gates: typecheck/lint clean; 898 tests; `/dashboard` + `/api/bot/override` ƒ; the inverted invariant + safe-module structural guard both green. — area: backend/ui/security — reversibility: medium.
