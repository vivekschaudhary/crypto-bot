...
- [2026-06-17] [Engineer] **Built (confirms the above).** Extracted `lib/ticks/run-tick.ts:runBotTick({source})` (cron floors / manual precise; returns `ran|skipped|duplicate|error`; writes the audit error row + emits traces — never throws). The cron `GET` is now a thin auth + result→Response mapper; its 26 route tests + the cron invariants pass UNCHANGED (the `"23505"` invariant assertion was swept to point at `run-tick.ts` — a `[cross-artifact-sweep-on-contract-shift]`). New `app/api/run-now/route.ts` (POST, override auth stack, `keyPrefix:"run-now"`) outside `/api/bot/**`. Enabled the Run Now button + `runNow()`/`runOutcomePhase()` (pure, unit-tested) → `done`/`skipped`/`error` feedback. Gates: typecheck/lint clean; 864 tests; build green (`/api/run-now` + `/api/cron/tick` both ƒ; `/dashboard` ƒ); the `/api/bot/**` no-orders invariant + cron orders-reachable invariant both green. — area: backend/ui — reversibility: medium.
