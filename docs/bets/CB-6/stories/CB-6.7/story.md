...
- [2026-06-18] [Engineer] **Built (confirms the above).** Migration 0008 + `orders.base_quantity` in schema; `OrderRowInsert`/`insertManualOrder` + the bot (run-tick) & manual write-paths persist `buildLimitOrder.baseQuantity`. New `lib/dashboard/paper-fills.ts:synthesizePaperFills` (dry_run orders → Fill[], price = amount/qty, NULL-qty skipped). `loadCockpitPnl` + `loadCockpitPosition` mode-switch on `env().LIVE_MODE` (dark → paper fills; live → real, unchanged) + carry a `paper` flag; invested is mode-aware (dark dry_run / live submitted). Paper badge on both cards. Existing CB-6.1/6.2 read-model tests pinned to LIVE_MODE=true (real path); new paper-mode + paper-fills + badge tests added. Gates: typecheck/lint clean; 913 tests; `/dashboard` ƒ; read-only invariant green. No page.tsx change (the `paper` flag flows via the read-model data). — area: backend/ui — reversibility: medium.
