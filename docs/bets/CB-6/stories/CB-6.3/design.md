...
- **NEXT ACTION block:** `NEXT ACTION` + the bot's latest decision for the pair — **BUY / SELL / HOLD** badge (colored, reusing CB-5.1 trace: buy `#1b5e20`, sell `#8a6d00`, hold `#444`) + the `reason` string **verbatim** below it. "Next action" = what the bot would do on its next evaluation given the current signals (the bot is deterministic/reactive — the latest decision IS the next action until signals change).
