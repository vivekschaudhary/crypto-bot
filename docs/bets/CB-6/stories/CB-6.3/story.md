...
- [ ] **AC 3 — Strategy-relative derivations (display only).** RSI zone from the **strategy's own thresholds**, not generic 30/70: `Oversold` = `rsi ≤ entry_rules.rsiThreshold`; `Overbought` = `rsi ≥ exit_rules.rsiThreshold`; else `Neutral`. Price-vs-MA: `Above`/`Below`/`At` (`lastClose` vs `ma`). Next Action = a badge **derived DB-only from the persisted decision + reason** — **AMENDED 2026-06-22** (dust phantom-sell fix; operator-approved): `BUY`/`SELL` (reason verbatim) · `HOLDING` (open-position hold, reason verbatim) · `WAITING TO BUY` (flat/dust/no-buy-signal hold → `Enters when RSI < <entry> (currently <rsi>, <zone>)`). Distinguishes open-vs-flat via the engine's `isOpenPositionHold(reason)` — **no Coinbase re-read** (so a transient fetch failure never rewrites the action). BUY/SELL/HOLDING reasons never paraphrased (refusal rule #5); the raw reason stays in `/dashboard/trace`.
...
- [2026-06-22] [Operator/Engineer] **AMENDED — Next Action badge derives BUY/SELL/HOLDING/WAITING-TO-BUY from the persisted signal (DB-only).** Live dust phantom-sell ([FIX-2026-06-22](../../../fixes/FIX-2026-06-22-dust-position-phantom-sell.md)): the original bare `HOLD` badge + verbatim reason surfaced a confusing "sell 90% of position" when flat (dust). Now a `hold` for an **open** position → `HOLDING` (reason verbatim); a `hold` while **flat** (no position / dust / no buy signal) → `WAITING TO BUY · Enters when RSI < <entry> (currently <rsi>, <zone>)`. Open-vs-flat is derived from the engine's persisted reason via `isOpenPositionHold(reason)` — **DB-only, no Coinbase re-read** (a first attempt deriving it from `loadCockpitPosition()` was a Codex BLOCKER: a transient account-fetch failure would rewrite a real SELL/HOLD into WAITING TO BUY — reverted). Operator chose the wording. BUY/SELL/HOLDING reasons stay verbatim (refusal rule #5); raw reason remains in `/dashboard/trace`. The engine's `MIN_SELLABLE_POSITION_USD` dust floor ensures dust is *decided* as a flat hold, so the persisted decision is the source of truth. — area: ux/correctness — reversibility: easy.
