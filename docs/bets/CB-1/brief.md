...
estimate:
  ...
  variance_vs_estimated: -16 days (shipped 2.3 weeks ahead of plan v5)
---
...
- **[CB-1.5](stories/CB-1.5/story.md) — Sign-out endpoint (`POST /api/auth/sign-out`).** Shipped 2026-06-02 via [PR #15](https://github.com/vivekschaudhary/crypto-bot/pull/15). 12 ACs covering verifySession + invalidateSession composition, cookie-clearing parity (`Set-Cookie: __compass_session=; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=0`), origin check (403), idempotency-via-401 posture (no "already signed out" branch), no proxy.ts modifications (sign-out path falls through to protected gate naturally — does not match `PUBLIC_EXACT` or any `PUBLIC_PREFIXES` entry), defense-in-depth re-verify (handler calls `verifySession` even though proxy already gated, per CB-1.4 Engineer Decision #5), typed-405 method discipline. **Engineer DRI Decisions:** extracted `SESSION_COOKIE_NAME` / `SESSION_TTL_SECONDS` / `buildSessionCookie` / `clearSessionCookie` to `lib/auth/cookie.ts` as single source of truth shared with `authenticate/finish/route.ts` (closes Risk #1 structurally — cookie-attribute drift between issuance and clearing now requires editing the shared helper); typed-405 pattern introduced via explicit method handlers (existing `/api/auth/*` routes still use Next.js default 405 — retrofit deferred as P3 Engineer Issue). 17 new Vitest tests (149 total, was 132) + fourth E2E in codebase (Codex AC 8 — sign-out → 200 + Set-Cookie clear → dashboard 302 with `?next=` → second sign-out 401). 3-commit review cycle across 2 rounds: round 1 surfaced 1 BLOCKER (status.md internal-consistency drift, same class as PR #11's v4-era hold-out); round 2 clean. **Codex security review: no findings.** Supplemental fresh-Agent Claude security read posted (4 LOWs, all sub-threshold per Codex); triggered a 5-PR A/B retrospective (workflow `wzkhajv1x`) — verdict `mixed-but-leans-supports`; Codex stays binding as security reviewer. Full methodology + per-PR findings + aggregate metrics + recommendations recorded in [docs/retros/2026-06-02-security-reviewer-ab.md](../../retros/2026-06-02-security-reviewer-ab.md). PM Issue 1 (challenge-storage approach) was closed by CB-1.2's Engineer DRI; PM Issue 2 (first-deploy onboarding UX) is CB-1.6's territory. Closes brief Open Issue on sign-out scope ("this session only" — NOT "all sessions for user").

- **[CB-1.6](stories/CB-1.6/story.md) — First-deploy onboarding UX + auth surfaces.** Shipped 2026-06-05 via [PR #17](https://github.com/vivekschaudhary/crypto-bot/pull/17) + security-follow-up [PR #18](https://github.com/vivekschaudhary/crypto-bot/pull/18). **The bet's first UI-surface story.** PR #17 (CB-1.6 proper): 12 ACs covering 4 new/changed surfaces (`/` mode-detecting landing, `/setup` + `/sign-in` mode-gated ceremony pages with WebAuthn, `/dashboard` minimal post-auth landing with sign-out button) + `proxy.ts` redirect-target update (`/` → `/sign-in`) + scaffold cleanup (delete `app/(dashboard)/page.tsx` route-group collision) + new shared `lib/auth/safe-next.ts` consumer-side `?next=` validator + `app/setup/lib/device-label.ts` UA→label derivation. First story with **Accessibility category NOT marked `n/a`** in the Standard Experience Checklist (AC 12). First **<5-min time-to-first-authenticated-dashboard guardrail** mechanically asserted via Playwright (AC 8). 246 Vitest + 5 E2E. **Codex's AC 8 E2E (`962e262`) surfaced 2 real production bugs** in setup-client + sign-in-client (`@simplewebauthn/browser@11` API drift + begin-response shape mismatch); static mocks masked them, real Playwright caught them. Same `[mechanical-output-verification]` pattern as PR #10's smoking-gun, one layer earlier (E2E vs production). 3-round Codex review cycle on PR #17 surfaced 6 BLOCKERs + 4 ISSUEs across rounds 1-2; round 3 clean. PR #18 (security follow-up, 4-round cycle): closed both MEDIUMs from the [operator-requested 2026-06-04 codebase security audit](../../retros/2026-06-04-codebase-security-audit.md) — M1 added cache + tag invalidation for the `count(auth_credentials)` query on the 3 pre-auth pages (defends `*/15` bot tick from postgres.js pool exhaustion); M2 consolidated the `safe-next` allowlist to single source of truth + corrected a false comment claiming "mirrored character-for-character" (closes CB-1.6 PM DRI Decision #3 deferral earlier than scheduled — the audit elevated drift from tech-debt to false-documented-invariant). 260 Vitest tests final. CB-1.6 closes both CB-1 brief Open Issues (first-deploy onboarding UX + device labeling UX).
...
5. ~~**Sign-out endpoint + cookie clearing**~~ — **shipped** via CB-1.5. (Originally forecast item #4.)
6. ~~**First-deploy onboarding UX**~~ — **shipped** via CB-1.6 (+ M1/M2 security follow-up via PR #18).

Original estimate ~6 stories at ~2-3 days each = ~2-3 weeks. **Final actuals: 7 story.md files (CB-1.1, CB-1.1.1, CB-1.2, CB-1.3, CB-1.4, CB-1.5, CB-1.6), all shipped, in 5 calendar days (2026-05-31 → 2026-06-05).** Plus 1 security follow-up PR (#18) addressing both MEDIUMs from the 2026-06-04 codebase audit. **CB-1 bet complete — 2.3 weeks ahead of plan v5's 06-21 estimated_end.** Variance attributable to: (a) absence of UX friction at n=1 single-operator (no stakeholder review cycles), (b) the multi-model A/B review discipline catching real-bugs-fast rather than after-deploy, (c) story decomposition tracking actual implementation complexity rather than padded fixed-week boundaries.
