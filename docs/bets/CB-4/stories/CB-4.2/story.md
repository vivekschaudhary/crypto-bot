...
**All three tables are EMPTY in production** (the cron has been heartbeat-only since 2026-05-31; zero `bot_ticks` / `signals` / `orders` rows exist). A reshape migration is therefore zero-data-risk. PM DRI Decision #1 below pins the new shape; the brief's Hypothesis sentence is amended in this same PR (cross-artifact sweep).
...
- [ ] **AC 2 — `lib/ticks/db.ts`** (new lib; server-only) exports the tick persistence layer:
  ...
  - `aggregateSessionTotals(sessionId): Promise<SessionTotals>` — sums `orders` rows (`source='bot'`, `side='buy'`, `status != 'failed'`, scoped to session) into `{dollarSpent, buyCount}`. **Note: returns zeros during the CB-4.2 window** (no `orders` writes until CB-4.3) — see PM DRI Decision #2.
...
- [2026-06-11] [PM] **Migration 0005 reshapes `signals` into per-asset decision rows; `bot_ticks` keeps its shape with aggregate semantics**
  - **Rationale (required):** The 0001-init schema predates multi-asset strategies (CB-1 era single-asset assumption). CB-4.1's shipped contract returns per-asset `{decision, reason, sizing}` — that data needs a per-asset home with nullable signal values (CB-4.0's null sentinel). All three event-log tables are EMPTY in production (heartbeat-only cron since 2026-05-31), so the reshape is zero-data-risk. Reshaping `signals` (rather than adding a fourth table) keeps the brief's Hypothesis sentence true ("N `signals` rows, one per asset evaluated") and avoids a redundant table whose rows would 1:1 shadow signals rows. `bot_ticks` is untouched: its `decision` CHECK constraint serves the aggregate, and `UNIQUE (session_id, tick_started_at)` stays load-bearing for cron-overlap defense.
  - **Area (required, tag):** architecture / schema-evolution / append-only-audit
  - **Alternatives considered (required):** new `tick_decisions` table alongside untouched `signals` (rejected — signals' kind/value shape would be dead weight; two tables for one concern); aggregate-only persistence in `bot_ticks` with per-asset detail in the log line only (rejected — CB-5 dashboard needs queryable per-asset rows; logs are retention-limited); full event-log redesign (rejected — out of bet scope; this is the minimal honest fix)
