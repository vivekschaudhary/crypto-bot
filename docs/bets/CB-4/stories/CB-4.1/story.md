...
- [ ] **AC 8 — Sell signal at asset with no position → hold**: if the exit-rule check would fire BUT `currentPosition === null` → decision is `hold` with reason `"hold: sell signal but no open position in BTC-USD"`. Defense: the operator could enter the strategy with an existing manual-bought position; the bot only sells what it knows it has via `currentPosition`. If the operator wants the bot to manage pre-existing positions, that's a CB-5 manual-override concern, not the decision engine's.
