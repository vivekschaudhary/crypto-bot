...
- [ ] **AC 13 — Real-path coverage, split across three layers (amended 2026-06-14; PM arbitration of a Reviewer-vs-Engineer dispute).** The original single-test wording ("places ONE real limit order … asserts the `orders` row … through the cron route") is **unachievable safely**: the route prices limits at `close × (1 ± 0.5%)` — *across* the spread — so a real order placed through the route fills near-instantly (real money spent before any cancel), and driving the real route also writes test rows into the production `bot_ticks`/`signals`/`orders` tables. The three concerns are therefore split:
  - **(a) Real-Coinbase placement round-trip** — triple-gated (`RUN_REAL_ORDER_TESTS=1`; skipped in CI): places ONE real limit order **far from market** (50% away, per CB-2.4) so it rests UNFILLED, asserts the real `coinbase_order_id`, then cancels. Wrapper-level (`getProduct` → `buildLimitOrder` → `placeOrder` → `cancelOrders`) — the only layer where a real order can be placed safely. **The empirical Q3 closure.**
  - **(b) Route-level end-to-end against REAL Coinbase reads, in DRY-RUN** — triple-gated: drives the real cron `GET` with `LIVE_MODE=false` + a forced-buy strategy + a capture-mocked DB (no prod writes), asserting the route builds a `dry_run` `orders` row from REAL market data (real candles → real RSI/MA → decision → row). Exercises `buildOrderRows` + the LIVE_MODE gate + the full read/persist pipeline "through the route" — safely (no placement, no prod pollution).
  - **(c) Route-level persistence shape (both modes)** — the existing `route.test.ts` unit tests already drive `GET` end-to-end (mocked Coinbase + capture-mocked DB) and assert the `dry_run` row AND the `submitted` row + `coinbase_order_id`. This is where the "asserts the `orders` row + `coinbase_order_id`" guarantee lives.
  - Each layer documents its manual/CI run posture.
