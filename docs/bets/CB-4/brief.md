...
- [2026-06-09] [PM] **Cron overlap → double-fire (same `tick_started_at`; duplicate decisions)**
  - **Likelihood (required):** LOW (Vercel cron is non-overlapping by default per Vercel docs; a tick that takes > 15 min would skip the next one rather than concurrently invoke)
  ...
  - **Mitigation (required):** DB-level `UNIQUE (session_id, tick_started_at)` constraint per [architecture.md DRI Risk: cron overlap](../../foundation/architecture.md) + [`0001-init.sql:62`](../../../db/migrations/0001-init.sql); the constraint rejects the second INSERT, so the second tick's transaction rolls back. CI grep test asserts the cron handler doesn't catch + swallow the constraint violation silently
  - **Area (required, tag):** concurrency / data-integrity

- [2026-06-09] [PM] **Coinbase rate-limit on candle fetches** — operator's up to 5 `selected_assets` × 1 candle fetch per tick = 5 RPS sustained max
  - **Likelihood (required):** low (CB-2.5's empirical finding: auth'd brokerage 30 RPS ceiling; bot's ~5 RPS at peak is 17% — close to but under the 10% guardrail at edge cases)
  - **Impact (required):** medium (rate-limit rejection → tick failure → bot_tick reliability metric breach at 99% target)
  - **Mitigation (required):** CB-2.5's `lib/coinbase/trace.ts` already logs `x-ratelimit-*` headers; the cron handler can degrade gracefully (per-asset try/catch; emit `hold` + reason for rate-limit-blocked assets). Investigate if Coinbase has a batch candle endpoint (multiple products in one request); if so, use it. Add 100ms inter-call delay between candle fetches if rate-limit pressure shows up in early production
  - **Area (required, tag):** external-api / rate-limit-budget
