...
- **CB-4.4 (~~maybe~~ DROPPED 2026-06-14)** — Take-profit polish + sell evaluation refinement. **Not built** — the "maybe" hedge resolved to DROP: CB-4.1's sell/exit logic has been firing correctly in production since CB-4.2 (2026-06-12), and CB-4.3 shipped the sell order path clean. No evidence of need for refinement. See [DRI Decision 2026-06-14: CB-4.4 dropped](#decisions). **CB-4 closes at 4 shipped stories.**
...
| 2026-06-14 | **CB-4 BET COMPLETE — all 4 stories shipped; CB-4.4 dropped** | CB-4.2 (cron tick) shipped + prod-verified 2026-06-13 (PR #63 — 84 ticks/0 errors/100% reliability/21h; migration 0005). CB-4.3 (`LIVE_MODE` gate + unified ledger) shipped 2026-06-14 (PR #65 — migration 0006; idempotent placement; ships DARK at `LIVE_MODE=false`; 1 cap-accounting BLOCKER + AC 9 leg-(c) + AC 13 split all closed; architecture `Order` entity amended upstream per Principle #16). **CB-4.4 (take-profit polish) DROPPED** — sell logic verified correct in prod since CB-4.2; no refinement needed (see DRI Decision 2026-06-14). **The bot runtime is fully in production.** Remaining is operator-side only: the dry_run ledger's first row awaits a natural buy/sell signal crossing (market all-hold since deploy — the write path is deployed + tested, just not yet exercised), and the `LIVE_MODE=true` flip is the deliberate operator ceremony behind the ≥60-dry-run-session guardrail (clock from 2026-06-12 21:45 UTC). CB-4's `key_metric` (≥99% tick reliability) is GREEN (100% observed). CB-5 (dashboard) promoted + approved in parallel; `/create-story CB-5` is next-on-deck. |
...
- [2026-06-14] [PM] **CB-4.4 (take-profit polish) DROPPED — CB-4 closes at 4 stories** (resolves the "maybe" forecast + CB-4.1 PM Decision #3's lean)
  - **Rationale (required):** CB-4.4 was hedged from the start as "maybe — only if the decision engine's sell logic needs separate iteration after CB-4.1 + CB-4.3 land." Production evidence says it doesn't: CB-4.1's exit/sell logic has been firing CORRECTLY in prod since CB-4.2 shipped (2026-06-12) — the verified ticks showed the full exit-rule path (`position open + rsi <= exit_threshold → hold`) and the AC-8 amended `exit rsi condition met … but no open position` reason rendering live + accurately. CB-4.3 shipped the sell order-placement path clean (limit pricing, base-size = fraction × held, idempotency) with no sell-logic defects surfaced in review. There is no observed need for take-profit refinement. Dropping is the honest call (mirrors the CB-3.4 fold pattern — don't build speculative polish without evidence of need). Any future sell-logic refinement would be a fresh, evidence-driven story, not a pre-committed CB-4.4.
  - **Area (required, tag):** scope / bet-closure
  - **Alternatives considered (required):** build CB-4.4 now (rejected — no evidence of need; speculative polish); keep it open indefinitely (rejected — leaving a phantom story open muddies CB-4's closure + the plan; close it cleanly with the door open for an evidence-driven follow-up)
  - **Reversibility:** trivial — a real take-profit issue surfaced during the dry-run window or post-live becomes a new story on its own merits
  - **Consequence:** **CB-4 is COMPLETE at 4 shipped stories (CB-4.0/4.1/4.2/4.3).** The bot runtime + `*/15` cron tick + signal eval + decision engine + dry-run/live order placement + unified ledger are all in production (shipping dark). The `LIVE_MODE=true` flip is the operator-only ceremony that remains, gated behind the ≥60-dry-run-session product guardrail (clock started 2026-06-12 21:45 UTC).
