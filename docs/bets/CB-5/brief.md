...
- **Decision-trace log** — chronological `bot_ticks` ⋈ `signals` (per-asset RSI/MA at decision time, decision, reason string, dry-run badge). The reason strings CB-4.1 made operator-readable render verbatim here.
...
- **CB-5.1** — **Decision-trace log view**: `bot_ticks` ⋈ `signals` chronological render with per-asset RSI/MA/decision/reason + dry-run badge. e2e: load → assert a tick's reason + signals render. Likely small-medium.
- **CB-5.2** — **Transaction-ledger view**: `orders` (dry_run + live), manual-vs-bot source, status. Closes Researcher Q1 (PnL scope). e2e: load → assert ledger rows render with source separation. Likely small-medium.
