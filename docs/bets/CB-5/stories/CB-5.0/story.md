...
- [ ] **AC 6 — READ-ONLY guardrail**: `/dashboard` performs zero writes — no INSERT/UPDATE against any table. CI grep test (extending the CB-4.2 append-only pattern) asserts `app/dashboard/**` contains no `INSERT`/`UPDATE` SQL and the read model imports no write fns. (Override write paths are CB-5.3.)
...
- [ ] **AC 8 — e2e Playwright spec** (`e2e/dashboard/live-state.spec.ts`): seed a `bot_sessions` row + bot `orders` rows (dry_run) + mock the Coinbase holdings read; load `/dashboard` → assert session status, holdings, activity (buy count + total invested), and the LIVE_MODE banner all render from the seeded data. Assert the banner text/colour matches `LIVE_MODE=false`. (A `LIVE_MODE=true` banner assertion may be a second spec case or deferred to CB-5.3 where the live path is exercised — Engineer DRI.)
