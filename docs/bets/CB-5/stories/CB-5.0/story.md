...
- [ ] **AC 8 — e2e Playwright spec** (`e2e/dashboard/live-state.spec.ts`): seed a `bot_sessions` row + bot `orders` rows (dry_run) + mock the Coinbase holdings read; load `/dashboard` → assert session status, holdings, activity (buy count + total invested), and the LIVE_MODE banner all render from the seeded data. **Both banner states MUST be asserted BY PLAYWRIGHT** — under `LIVE_MODE=false` AND under `LIVE_MODE=true` — per the [CB-5 brief LIVE_MODE guardrail](../../brief.md) verbatim: "Playwright asserts it under both `LIVE_MODE=false` and `=true`." This is SAFE to exercise exhaustively in e2e — the banner is a pure render of `env().LIVE_MODE` and `/dashboard` is read-only (no order placement; the live path is not reachable from this surface), so booting the app under `LIVE_MODE=true` for the banner case risks nothing. Mechanism is Engineer DRI **but it MUST be Playwright** (e.g. a Playwright project/config that boots the server with `LIVE_MODE=true` for the LIVE-banner spec, or a per-test env override that re-boots accordingly) — **no non-Playwright substitute** (a component-level/unit render assertion does NOT satisfy this guardrail). Neither state may be deferred. (PR #68 round-1: the `true` case was optional/deferred → made mandatory. Round-2: a component-test escape hatch remained → removed; both states are Playwright-exercised, full stop.)
