...
- [2026-06-19] [Engineer] **Built (confirms the above).** New `app/globals.css` (shell + `@media (max-width:767px)` only) imported in `app/layout.tsx` + `export const viewport`; `app/dashboard/layout.tsx` → async server component (flex `[sidebar|content]`, fetches `device_label` via the cockpit's `x-session-user-id`→DB read) rendering `DashboardSidebar`; new `dashboard-sidebar.tsx` (title + 6 nav + `activeNavKey` + footer device-label + single `SignOutClient`); removed `dashboard-tabs.tsx` (+ its test); removed the per-page `SignOutClient` from cockpit/trace/ledger (+ cockpit `chromeStyle`). Side-effect: the shared layout's `headers()`+DB read makes equity/mutual-funds dynamic (ƒ) too — correct for auth-gated routes (CB-5.1 lesson). device-line uses a single-string template literal (split-text-node). Gates: typecheck/lint clean; 916 tests; build green; `.dashboard-shell` confirmed in an emitted CSS asset; read-only invariant green; `/dashboard` ƒ. — area: ui/layout — reversibility: medium.
