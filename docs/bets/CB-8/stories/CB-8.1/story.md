...
- [ ] **AC 2b — Persistence + no flash.** State persists in `localStorage` (`sidebar-collapsed`; `1`=collapsed). Applied **pre-paint** via an inline `<html>` script in `app/layout.tsx` (reads localStorage → sets `data-sidebar-collapsed`) → no expand→collapse flash on reload. The script intentionally mutates `<html>` before hydration (the server can't read localStorage), so `<html>` carries **`suppressHydrationWarning`** — the sanctioned no-flash pattern (next-themes etc.); it scopes to `<html>`'s own attributes only (child mismatches still surface). No hydration WARNING; hydration intact.
...
- [2026-06-20] [Architect/Engineer] **localStorage + pre-paint `<html>` script + CSS `[data-sidebar-collapsed]`** (per the approved architecture) — no flash; the toggle is DOM/storage-driven, React state only for `aria-expanded`. — area: client-state — reversibility: easy.
- [2026-06-20] [Engineer] **`suppressHydrationWarning` on `<html>` (BLOCKER closure).** Codex flagged a real React hydration-mismatch warning: the pre-paint script adds `data-sidebar-collapsed` to `<html>` but the SSR `<html>` lacks it (localStorage is client-only — the server can't render it). Fixed with `suppressHydrationWarning` on `<html>` — React's designated API for intentional pre-paint mutations (the next-themes pattern); it scopes to `<html>`'s OWN attributes (one level — child mismatches still warn). The earlier "no hydration mismatch" wording is corrected to "no hydration WARNING; the expected single-attr difference is suppressed; hydration intact." Cookie-based SSR-readable persistence was the heavier alternative (rejected — re-litigates the approved localStorage decision for a per-browser UI pref). — area: client-state — reversibility: easy.
