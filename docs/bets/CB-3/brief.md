...
- **CB-3.3 — Strategy authoring form UI + save action + activation (folds CB-3.4)** ✅ SHIPPED 2026-06-09 (PR [#49](https://github.com/vivekschaudhary/crypto-bot/pull/49) + 4 post-merge polish PRs [#50](https://github.com/vivekschaudhary/crypto-bot/pull/50)/[#51](https://github.com/vivekschaudhary/crypto-bot/pull/51)/[#52](https://github.com/vivekschaudhary/crypto-bot/pull/52)/[#53](https://github.com/vivekschaudhary/crypto-bot/pull/53)) — `/dashboard/strategy` Server Component + Client Component generic form (over **asset-class primitives + `labels: StrategyFormLabels` prop** — amended PR #50 from the original `adapter: AssetAdapter` prop after RSC serialization defect; further amended PR #53 with the labels prop for true equity-app portability per brief PM Decision #6) + server action that validates + INSERTs strategy + wires supersession + UPSERTs `bot_sessions.active_strategy_id` + emits structured log. **First Playwright e2e of CB-3** — operator authors a strategy → save → reload → strategy persists; second spec verifies supersession on revise. CB-1.6 lesson applied in force. Form is asset-class-agnostic at both the data-shape AND copy levels (see [architecture Decision #6 amendments 2026-06-09](architecture.md#6-form-architecture--server-component-shell--client-component-generic-form)); CB-3 ships only the crypto-coinbase adapter wired into the route via `COINBASE_LABELS` at page.tsx. **Folded CB-3.4** per PM Decision at `/create-story CB-3.3` 2026-06-08. **3 production-only-defect lessons named** during the post-merge cycle (`[rsc-prop-serialization]`, `[server-action-file-export-purity]`, `[empty-numeric-input-zero-trap]`) — see [retro 2026-06-09](../../retros/2026-06-09-cb-3-production-only-defects-retro.md).
...
**CB-3 SHIPPED 2026-06-09 — 1.5 calendar days end-to-end vs the 3 wk medium-confidence brief-approval estimate (~17x faster)**, even with 3 production-only defects + 1 portability refactor in the post-merge cycle. Brief-approval-time forecast (4-6 calendar days total per CB-2's 0.6 days/story velocity) was conservative by ~3x; the actual cadence was tighter still because CB-3.0/.1/.2 shipped same-day (2026-06-08) and CB-3.3 cleared the next day. The empirical velocity feeds the next [`/plan`](../../foundation/plan.md) refresh per the retro's meta-observation that brief-approval medium-confidence estimates have been consistently 5-15x slow across CB-1/CB-2/CB-3. See [docs/retros/2026-06-09-cb-3-production-only-defects-retro.md](../../retros/2026-06-09-cb-3-production-only-defects-retro.md) for the post-bet analysis.

## Scan summary

- **Last scanned:** n/a (no `/scan CB-3` run was fired during the bet's 1.5d shipping cycle — operator-directed velocity; scan deferrable to post-ship `/measure` cycle per [retro 2026-06-09](../../retros/2026-06-09-cb-3-production-only-defects-retro.md))
- **Current phase:** **SHIPPED 2026-06-09** (Build complete; awaiting `/measure CB-3` cron pickup for the Operate phase). 4 of 4 stories merged via PRs [#45](https://github.com/vivekschaudhary/crypto-bot/pull/45) + [#47](https://github.com/vivekschaudhary/crypto-bot/pull/47) + [#48](https://github.com/vivekschaudhary/crypto-bot/pull/48) + [#49](https://github.com/vivekschaudhary/crypto-bot/pull/49); plus 4 post-merge polish PRs [#50](https://github.com/vivekschaudhary/crypto-bot/pull/50)/[#51](https://github.com/vivekschaudhary/crypto-bot/pull/51)/[#52](https://github.com/vivekschaudhary/crypto-bot/pull/52)/[#53](https://github.com/vivekschaudhary/crypto-bot/pull/53). 9-PR squash-race streak clean. 554 unit tests + 2 Playwright e2e specs at close.
- **Open findings:** n/a (no `/scan` artifact; 3 production-only-defect lessons named in the retro instead — `[rsc-prop-serialization]` + `[server-action-file-export-purity]` + `[empty-numeric-input-zero-trap]` + a 5th-instance cross-artifact-drift convention candidate `[cross-artifact-sweep-on-contract-shift]` flagged for AGENTS.md promotion).
- **Blocking advance:** no — CB-4 (bot runtime) unblocks.
- **Full report:** no `/scan` report; see [docs/retros/2026-06-09-cb-3-production-only-defects-retro.md](../../retros/2026-06-09-cb-3-production-only-defects-retro.md) for the substantive post-ship analysis.
...
| 2026-06-09 | CB-3.3 build shipped + 3 production-only post-merge defects | CB-3.3 build merged via [PR #49](https://github.com/vivekschaudhary/crypto-bot/pull/49) (squash `0a4c864`). Operator hit `/dashboard/strategy` in production within minutes; surfaced 3 defects invisible to local `pnpm dev` + `pnpm build` + 541 passing tests: **(a)** [PR #50] React Server Components cannot serialize regular function methods across the RSC boundary — only Server Actions can cross. Passing `adapter: AssetAdapter` as a prop caused 3 `E{digest:...}` broken-reference errors at runtime. Fix: route layer passes resolved primitives, not the adapter object. Lesson named: **`[rsc-prop-serialization]`**. **(b)** [PR #51] `"use server"` files can only export async functions. My dead re-export `export { VALIDATION_ERROR_CODES, errorCodeToFieldPath }` violated the rule with production-only error. Fix: remove the dead re-export. Lesson named: **`[server-action-file-export-purity]`**. **(c)** [PR #52] HTML number inputs render `Number("") === 0`, leaving "0" stuck in cleared fields. Operator-reported. Fix: NaN sentinel + Number.isFinite gate on submit. Lesson named: **`[empty-numeric-input-zero-trap]`**. All three closed forward without formal reopen ceremony per `/build` Post-merge bugs rule. |
| 2026-06-09 | CB-3.3 polish PR — labels prop for true equity-app portability | Operator confirmed the design intent: "the change we made is a package that can be pulled into another program where I have equity — the same strategies might apply so it should be asset agnostic." Verified extraction-readiness via the existing no-coupling invariant tests (all 4 still pass). Surfaced 3 remaining crypto-flavored copy strings hardcoded in form-client.tsx + defaults.ts. [PR #53] lifted all 3 to a route-layer `labels: StrategyFormLabels` prop; new `COINBASE_LABELS` constant in page.tsx; defaults.ts default error copy flipped `"cryptos" → "assets"` (the crypto-coinbase verbatim copy.md string preserved via `labels.errorOverrides`); new pure `formatAsOfStamp(date)` helper extracted for cross-app reuse. The equity-app extraction is now a literal half-day find/replace per [PM DRI Decision #6](#decisions) — `EQUITY_LABELS` at equity's page.tsx + an equity adapter (Alpaca/Tradier/etc.) implementing `AssetAdapter` is all that's needed. **Cross-artifact-drift recurred on PR #50 + PR #53** — Codex caught both rounds: code shipped a widened/changed prop contract but story.md AC 3 + architecture.md Decision #6 needed the same-PR amendment. Both swept; pattern worth surfacing in a future `/retro`. |
| 2026-06-09 | CB-3 BET SHIPPED | All 4 stories shipped (CB-3.0 + CB-3.1 + CB-3.2 + CB-3.3 after CB-3.4 fold) + 4 post-merge polish PRs (#50/#51/#52/#53). **1.5 calendar days end-to-end vs 3 wk medium-confidence brief-approval estimate (~17x faster)** — even with 3 production defects + 1 portability refactor in the cycle. 9-PR squash-race streak clean (pre-merge `headRefOid` ritual held flawlessly). 554 tests at close. Strategy authoring loop end-to-end in prod at `/dashboard/strategy`. Researcher Q1 seeded empirically; Q2 closed. CB-4 (bot runtime + `*/15` cron tick) unblocks. The 3 production-only-defect lessons + the cross-artifact-drift recurrence both worth surfacing in a future `/retro` per operator direction. |
