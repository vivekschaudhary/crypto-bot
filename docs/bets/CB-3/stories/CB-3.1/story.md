...
- [ ] **AC 3** — `rankByVolume(assets)` sorts the given assets by Coinbase's `volume_24h` (descending). Implementation: calls `getProducts()` to build a `product_id → volume_24h` lookup map; for each asset, looks up its identifier's volume. **`volume_24h` is a string per Coinbase** — the adapter must `parseFloat` defensively; non-parseable values (`NaN`, missing entry, empty string) sort last (treated as volume 0). Returns a NEW sorted array (input not mutated).
...
2. **`volume_24h` parsing — `parseFloat` defensively; non-parseable → 0 → sort last.** Coinbase returns the field as a string per CB-2.2's schema (`volume_24h: z.string().min(1)`). Real-world cases the adapter must handle: legitimate numeric strings ("12345.67"), missing entries (product not in the lookup map), empty strings (Coinbase might return ""), and NaN-producing inputs ("not a number"). All non-parseable cases → 0 → sort last. Engineer commits this as Decision #2; unit test covers each case.
...
- [2026-06-08] [PM] **Top-5 anchor test asserts BTC-USD presence; AVOIDS brittle exact-order assertions**
  - **Rationale (required):** Top-5 order shifts run-to-run as Coinbase volume fluctuates intraday. Asserting exact order ("BTC-USD > ETH-USD > SOL-USD > ...") would fail randomly even when the adapter is correct. Asserting STRUCTURE (5 items, valid Asset shape) + a SINGLE canonical anchor ("BTC-USD present in top-5") gives reliable signal without false negatives. BTC-USD is the only crypto where "should always be in top-5" is a safe assumption.
  - **Area (required, tag):** test-discipline / integration-test-stability
  - **Alternatives considered (required):** assert specific top-5 set (rejected — would fail on any market event); assert top-3 instead of top-5 (rejected — narrower window; same brittleness class); assert top-5 LENGTH only (rejected — would pass even if the adapter returned random products in wrong order); skip the anchor assertion (rejected — the WHOLE POINT of integration testing is to verify against reality)
  - **Reversibility:** trivial — adjust anchor if BTC-USD ever drops out (vanishingly unlikely in MVP timeframe)

### Risks

- [2026-06-08] [PM] **`volume_24h` precision edge cases — string may have unexpected formats**
...
- [2026-06-08] [PM] **Live-API integration test may surface Coinbase contract drift the adapter doesn't yet handle**
  - **Likelihood (required):** low (CB-2.2's schema uses `.passthrough()` so new fields don't break parsing; the only failure mode is if `volume_24h` / `quote_currency_id` / `product_type` shape changes incompatibly)
