...
- [ ] **AC 3 — Generic form over `AssetAdapter` prop.** `_form.tsx` is a Client Component that accepts `adapter: AssetAdapter` prop (the interface from CB-3.0); renders asset-class-agnostic field sections (Name → Asset Selector → Entry Rules → Exit Rules → Caps → Submit/Cancel). The form itself does NOT import from `@/lib/strategy-coinbase/*` — wiring of the concrete `makeCoinbaseAdapter()` happens at the route layer (`page.tsx`). This is the pluggability invariant CB-3.0's mock equity adapter test proved + CB-3.1's real adapter implements.
