...
- [2026-06-06] [Engineer] **EdDSA path against Coinbase `/api/v3/brokerage/*` endpoints is unverified** — surfaced by Codex PR #26 round-1 BLOCKER (**RESOLVED 2026-06-07**)
  ...
  - **Resolution (filled 2026-06-07):** **RESOLVED** earlier than planned (was deferred to CB-2.5; closed during CB-2.3 instead). [CB-2.3 PR #34](https://github.com/vivekschaudhary/crypto-bot/pull/34) Engineer DRI Decision #8 = `pass`: all 3 live integration tests pass against real Coinbase `/api/v3/brokerage/*` endpoints with the operator's actual key format. No 401s; auth path works end-to-end. Coinbase brokerage docs that warned of Ed25519 401 are either stale or apply to a different key surface than what the operator has provisioned. The JSDoc caveat in [`lib/coinbase/jwt.ts`](../../../../lib/coinbase/jwt.ts) is now historical (preserved per Compass append-only convention); CB-2.4 + CB-2.5 inherit the auth path with confidence. **No CDP key regeneration needed.**
