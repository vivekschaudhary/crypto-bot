...
- [CB-2 brief § In scope](../../brief.md#in-scope) — names `market.ts` with `getProducts`, `getProductStats`, `getProductCandles`. Note: this story's AC 1 ships `getProduct(productId)` rather than a separate `getProductStats` because Coinbase's product-detail endpoint includes 24h volume + stats in the same response; no separate stats endpoint exists at the Advanced Trade v3 surface (Engineer verifies during build).
