...
- **CB-3** (strategy authoring): calls `getProducts()` + `getProductStats()` to surface available trading pairs and compute the top-5-by-global-24h-volume that the operator then selects from.
...
- **`lib/coinbase/`** typed wrapper module:
  ...
  - `lib/coinbase/market.ts` — public market data: `getProducts()`, `getProductStats(productId)` (24h volume for top-5 computation in CB-3), `getProductCandles(productId, granularity, window)` (signal-input for CB-4)
