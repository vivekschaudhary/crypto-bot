...
- **CB-2.4 — `orders.ts` for authenticated WRITES (real-money)** — `placeOrder({productId, side, orderConfiguration, clientOrderId?})`, `cancelOrders({orderIds: string[]})`; **LIVE_MODE-free architectural invariant** (CB-4 owns the gate); the typed surface CB-4 (bot tick) + CB-5 (dashboard override) write to. Two methods translate from idiomatic wrapper args to Coinbase's actual POST `/orders` discriminated-union body (`market_market_ioc` + `limit_limit_gtc` variants) and POST `/orders/batch_cancel` (no singular cancel endpoint exists). `client_order_id` (idempotency key) auto-generated via `crypto.randomUUID()` from `node:crypto` if absent. Parameter shapes reflect the 2026-06-08 PM DRI Decision above (applied PROACTIVELY from web-researched docs before drafting — no Codex round-1 BLOCKER expected). Integration test requires `RUN_REAL_ORDER_TESTS=1` opt-in flag (places real orders against operator's account; recommended strategy: limit-far-from-market + post_only + cancel symmetry). **`ready` 2026-06-08** via [PR #37](https://github.com/vivekschaudhary/crypto-bot/pull/37); see [story.md](stories/CB-2.4/story.md).
...
- **Last scanned:** n/a (no scan-report yet — first scan happens after the first story shipped per `/scan` pattern; CB-2.1 + CB-2.2 + CB-2.3 all shipped 2026-06-06 → 2026-06-07 but `/scan CB-2` not yet fired)
- **Current phase:** in-build (CB-2.1 + CB-2.2 + CB-2.3 shipped via [PR #26](https://github.com/vivekschaudhary/crypto-bot/pull/26) + [PR #32](https://github.com/vivekschaudhary/crypto-bot/pull/32) + [PR #34](https://github.com/vivekschaudhary/crypto-bot/pull/34) (+ [PR #35](https://github.com/vivekschaudhary/crypto-bot/pull/35) missed-fixes restore); 3 of ~5 stories done; CB-2.4 story `ready` 2026-06-08; `/build CB-2.4` is the next workflow. EdDSA brokerage caveat RESOLVED.)
