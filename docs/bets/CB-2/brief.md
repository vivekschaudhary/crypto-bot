...
- **CB-2.5 — `trace.ts` — request observability + rate-limit awareness** — structured-JSON `console.log` per request (`{method, path, status, duration_ms, rate_limit?}`); Vercel runtime log collection ingests for the success-rate metric derivation. **Sentry SDK install deferred** (operator confirmed 2026-06-08: rich observability not needed at this time; structured logs satisfy the key_metric requirement; @sentry/nextjs can land in a follow-up `/ops` PR if surfaced as load-bearing later). Closes **Researcher Open Question #1** (rate-limit headers — empirical discovery at integration test) and **Researcher Open Question #3** (CDP JWT rotation behavior of direct path — code-review confirms mint-per-request from env values; no process-restart needed). Modifies `lib/coinbase/client.ts` for the first time since CB-2.1 (small + additive + LIVE_MODE-free). **LAST CB-2 STORY** — when this ships, CB-2 is done and CB-3 (strategy authoring) unblocks. **`ready` 2026-06-08** via [PR #40](https://github.com/vivekschaudhary/crypto-bot/pull/40); see [story.md](stories/CB-2.5/story.md).
...
- **Last scanned:** n/a (no scan-report yet — first scan happens after the first story shipped per `/scan` pattern; CB-2.1 → CB-2.4 all shipped 2026-06-06 → 2026-06-08 but `/scan CB-2` not yet fired)
- **Current phase:** in-build (CB-2.1 + CB-2.2 + CB-2.3 + CB-2.4 shipped via [PR #26](https://github.com/vivekschaudhary/crypto-bot/pull/26) + [PR #32](https://github.com/vivekschaudhary/crypto-bot/pull/32) + [PR #34](https://github.com/vivekschaudhary/crypto-bot/pull/34) + [PR #39](https://github.com/vivekschaudhary/crypto-bot/pull/39); 4 of ~5 stories done; **CB-2.5 story `ready` 2026-06-08 — LAST CB-2 STORY**; `/build CB-2.5` is the next workflow. EdDSA brokerage caveat RESOLVED. Real-money write path verified.)
