...
- **CB-4** (bot runtime): calls `getAccountBalances()`, `getProductCandles()` for signal computation, and conditionally `placeOrder()` / `cancelOrders()` (gated on `LIVE_MODE` **inside CB-4**, not the wrapper). (The 2026-06-08 PM DRI Decision renamed the cancel wrapper from singular `cancelOrder(orderId)` to plural `cancelOrders({orderIds: string[]})` to match Coinbase's actual `batch_cancel` endpoint.)
...
- **`lib/coinbase/`** typed wrapper module:
  ...
  - `lib/coinbase/orders.ts` — authenticated write: `placeOrder({productId, side, orderConfiguration, clientOrderId?})`, `cancelOrders({orderIds: string[]})`. **No `LIVE_MODE` check inside.** Consumer (CB-4) owns the policy. **Amended 2026-06-08 (during `/create-story CB-2.4`) — see PM DRI Decision below:** the original wording said `placeOrder({productId, side, amount, type})` and `cancelOrder(orderId)`, but Coinbase Advanced Trade's `/orders` endpoint requires a discriminated-union `order_configuration` body (not an abstract `{amount, type}` pair) plus a required `client_order_id` idempotency key; and the cancel endpoint is `batch_cancel` with `order_ids: string[]` (no singular cancel exists). The wrapper renames accordingly to match Coinbase's actual API surface.
...
- **Rate-limit awareness:** wrapper's `trace.ts` extracts `x-ratelimit-*` headers (case-insensitive; confirmed empirically 2026-06-08 during CB-2.5 build that Coinbase Advanced Trade **auth'd** brokerage endpoints DO return `x-ratelimit-limit`, `x-ratelimit-remaining`, `x-ratelimit-reset` per response with a 30 RPS global limit; **public** market endpoints don't return these headers). The captured values are emitted in the structured trace log line for downstream observability tools to query. **Active 25%-utilization warning emission via Sentry breadcrumb is deferred** per operator confirmation 2026-06-08 — Sentry SDK install is not in CB-2.5 scope; a follow-up `/ops` PR (or DB persistence layer) can add active warnings if/when the metric becomes load-bearing for a check-in. At the operator's ~96 ticks/day cron rate (~0.001 RPS), utilization stays vastly under the 10%-of-ceiling guardrail; no back-off logic required for normal operation.
...
1. ~~**Coinbase rate-limit headers**~~ — **CLOSED 2026-06-08 by CB-2.5 (PR #41)** empirical live-API discovery. Auth'd `/api/v3/brokerage/*` endpoints DO return `x-ratelimit-limit`, `x-ratelimit-remaining`, `x-ratelimit-reset` per response (RFC 7234-ish format; 30 RPS global limit observed: `x-ratelimit-limit: "30, 30;w=1;name=\"global_limit\""`). Public market endpoints (`/api/v3/brokerage/market/*`) do NOT return these headers. `trace.ts:extractRateLimit` captures the values into the structured trace log line when present. The defensive 25%-warning emission via Sentry breadcrumb is deferred (operator confirmed Sentry-out-of-scope; the captured trace data lives in Vercel runtime logs and is queryable downstream). At the operator's ~96 ticks/day cron rate (~0.001 RPS), utilization stays vastly under the 10%-of-ceiling guardrail; no back-off logic needed.
...
3. ~~**CDP JWT key rotation cadence**~~ — **CLOSED 2026-06-08 by CB-2.5 (PR #41)** code-review resolution. `lib/coinbase/jwt.ts:mintJWT` reads `env()` per call (NOT cached at module load), so inter-request rotation works automatically: the next request after the operator updates Vercel env vars picks up the new key. **No process-restart shim needed.** Mid-request rotation is not possible (a single request uses the key active when `mintJWT` ran) but irrelevant — each retry is a fresh request and remints. Documented inline in `jwt.ts` JSDoc.

## Research findings

**All three Researcher items now closed.** Item 2 (SDK comparison) was closed by the CB-2.1 Engineer DRI Decision (no SDK; direct REST + JWT). Items 1 (rate-limit headers) and 3 (CDP JWT key rotation behavior of our direct path) were both closed by CB-2.5 build 2026-06-08 (PR #41): item 1 by empirical live-API discovery (auth'd brokerage returns `x-ratelimit-*` with 30 RPS limit; public market doesn't); item 3 by code-review confirmation that `jwt.ts:mintJWT` reads `env()` per call, so inter-request rotation auto-works without a process-restart.
...
- **CB-2.3 — `accounts.ts` for authenticated reads** — `getAccountBalances()`, `getAccount(uuid)`, `getAccountTradeHistory({productIds?: string[], start?, end?, cursor?, limit?})`. **SHIPPED 2026-06-07** via [PR #34](https://github.com/vivekschaudhary/crypto-bot/pull/34) (commit `d642b99`) + missed-fixes restoration via [PR #35](https://github.com/vivekschaudhary/crypto-bot/pull/35) (commit `925a187`). Ships `lib/coinbase/{accounts,account-schemas}.ts` + 17 unit tests + 3 gated integration tests against real Coinbase brokerage. 8 Engineer DRI Decisions including: auto-paginate at limit=250, RFC3339 timestamps, 9-field-required AccountSchema matching Coinbase docs, sensitive-data anti-echo, fail-loud pagination-contract-violation. **EdDSA brokerage caveat RESOLVED** end-to-end with operator's actual key — Decision #8 = `pass`. Codex round-1 BLOCKER: singular `productId` → plural `product_ids` array per docs (silent-match-all risk avoided); restoration in PR #35 also added sentinel-empty-filter integration-test pattern.
- **CB-2.4 — `orders.ts` for authenticated WRITES (real-money)** — `placeOrder({productId, side, orderConfiguration, clientOrderId?})`, `cancelOrders({orderIds: string[]})`; **LIVE_MODE-free architectural invariant** (CB-4 owns the gate); the typed surface CB-4 (bot tick) + CB-5 (dashboard override) write to. Discriminated-union `order_configuration` body (`market_market_ioc` + `limit_limit_gtc` variants); `client_order_id` (idempotency key) auto-generated via `crypto.randomUUID()` from `node:crypto` if absent. **SHIPPED 2026-06-08** via [PR #39](https://github.com/vivekschaudhary/crypto-bot/pull/39) (commit `51196f2`). Ships `lib/coinbase/{orders,order-schemas}.ts` + 17 unit tests + 3 gated integration tests against real Coinbase write endpoints. 6 Engineer DRI Decisions including the OrderResponseSchema success-gated envelope (`.superRefine()` ensures success_response REQUIRED when success=true; error_response REQUIRED when success=false) — Codex round-1 BLOCKER. **Live write path verified end-to-end**: limit BUY 50% below market with post_only=true accepted; cancelOrders returned success; no fill. Codex security review clean. **Coinbase rejects far-from-market limits** with `PREVIEW_LIMIT_PRICE_TOO_FAR_FROM_MARKET` — 50% below market is within tolerance (calibration discovery; relevant for CB-4 bot tick).
- **CB-2.5 — `trace.ts` — request observability + rate-limit awareness** — structured-JSON `console.log` per request (`{method, path, status, duration_ms, rate_limit?}`); Vercel runtime log collection ingests for the success-rate metric derivation. **SHIPPED 2026-06-08** via [PR #41](https://github.com/vivekschaudhary/crypto-bot/pull/41) (commit `e343f7e`). Ships `lib/coinbase/{trace}.ts` + `client.ts` integration + 15 unit tests + 3 live integration tests. **CB-2.5's empirical live-API test was the load-bearing closure for Researcher Open Question #1** (rate-limit headers): auth'd brokerage endpoints DO return `x-ratelimit-*` headers (30 RPS global limit per RFC 7234-ish format observed `"30, 30;w=1;name=\"global_limit\""`); public market endpoints do NOT. Researcher Open Question #3 (CDP JWT rotation behavior of direct path) closed via code-review: `jwt.ts:mintJWT` reads `env()` per call; inter-request rotation auto-works without process-restart. Sentry SDK install deferred per operator. **CB-2 is DONE — CB-3 (strategy authoring) unblocks.**
...
- **Last scanned:** n/a (no scan-report yet; CB-2 shipped without a `/scan CB-2` firing. If we ever revisit, a post-ship `/scan CB-2` would now have 5 shipped stories as the corpus)
- **Current phase:** **DONE 2026-06-08** (actual_duration_days: 3 vs estimated duration_weeks: 1). All 5 stories shipped via [PR #26](https://github.com/vivekschaudhary/crypto-bot/pull/26) + [PR #32](https://github.com/vivekschaudhary/crypto-bot/pull/32) + [PR #34](https://github.com/vivekschaudhary/crypto-bot/pull/34) + [PR #39](https://github.com/vivekschaudhary/crypto-bot/pull/39) + [PR #41](https://github.com/vivekschaudhary/crypto-bot/pull/41). Sub-bullets verified: EdDSA brokerage caveat RESOLVED (CB-2.3), real-money write path verified end-to-end (CB-2.4), rate-limit-headers + CDP JWT rotation Researcher items RESOLVED (CB-2.5). **CB-3 (strategy authoring) is the next critical-path bet — promoted from stub via `/create-brief CB-3` 2026-06-08.**
...
- [2026-06-08] [PM] **`orders.ts` API surface — `placeOrder` + `cancelOrders` shapes redesigned to match Coinbase's actual endpoint contracts** — surfaced during `/create-story CB-2.4` (applying the lesson tagged in the prior Decision)
  - **Rationale (required):** the original [In scope](#in-scope) bullet for `orders.ts` listed `placeOrder({productId, side, amount, type})` and `cancelOrder(orderId)`. Three issues against Coinbase's actual API surface:
    1. **`placeOrder` `{amount, type}` → `{orderConfiguration: OrderConfiguration}`**: Coinbase's POST `/api/v3/brokerage/orders` body uses a discriminated-union `order_configuration` field with variants like `market_market_ioc` (quote_size for buy-by-USD OR base_size for sell-by-asset-amount) and `limit_limit_gtc` (base_size + limit_price + post_only). An abstract `{amount, type}` pair can't express the quote-vs-base-size distinction or limit-order parameters like `post_only`. The discriminated-union signature pushes the variant choice to the consumer (CB-4 / CB-5), matching Coinbase's actual API.
    2. **`placeOrder` gains `clientOrderId?: string`**: Coinbase REQUIRES `client_order_id` (idempotency key) in the request body — not an optional field. Wrapper auto-generates a UUID v4 via `node:crypto`'s `randomUUID()` when caller doesn't provide one. CB-4's bot tick can pass an explicit ID to retry-after-network-failure without double-placement.
    3. **`cancelOrder(orderId)` → `cancelOrders({orderIds: string[]})`**: Coinbase's cancel endpoint is `POST /api/v3/brokerage/orders/batch_cancel` accepting `{order_ids: string[]}` — there is NO singular cancel endpoint. A singular wrapper would have been an internal-batches-of-one complication for no semantic gain; the plural-array surface matches the API directly and naturally supports multi-cancel for CB-5 dashboard override if surfaced.
  - **Area (required, tag):** product / api-surface / contract-correctness / real-money
  - **Alternatives considered (required):** keep abstract `{amount, type}` and translate internally to `order_configuration` (rejected — wrapper would have to make policy decisions about which variant to pick from `amount`, inviting bugs); add a separate `placeMarketOrder` + `placeLimitOrder` (rejected — fans out methods for one endpoint; doesn't scale if more variants are added); auto-generate `clientOrderId` only never accept caller-provided (rejected — breaks CB-4's retry-with-same-ID idempotency story); keep singular `cancelOrder(orderId)` as a 1-element wrapper around `cancelOrders` (rejected — slight code clutter for no semantic gain; if a consumer wants singular it can wrap `cancelOrders` inline)
  - **Reversibility:** easy NOW (rename in CB-2.4 story.md AC 1 + brief In scope before any code lands). Hardens once CB-4 or CB-5 consumes the API.
  - **Lesson applied PROACTIVELY:** this Decision was made at `/create-story CB-2.4` time after web-researching Coinbase docs UP-FRONT (per the lesson tagged in the prior productIds Decision). The verifications confirmed: discriminated-union body shape, required `client_order_id`, plural `batch_cancel`. **No Codex round-1 BLOCKER expected on these contract values** — pattern works.
  - **Forward lesson tag:** for CB-2.5 (`trace.ts`), verify rate-limit-header name(s) (e.g., `X-RateLimit-Remaining` vs alternative naming) + Sentry breadcrumb shape conventions against current docs before drafting that story.
  - **Surfaced by:** `/create-story CB-2.4` web-research of [Create Order docs](https://docs.cdp.coinbase.com/api-reference/advanced-trade-api/rest-api/orders/create-order) + [Cancel Orders docs](https://docs.cdp.coinbase.com/api-reference/advanced-trade-api/rest-api/orders/cancel-order).
...
- [2026-06-06] [PM] **Coinbase rate-limit headers may not be returned by the Advanced Trade `/api/v3/brokerage/*` endpoints** (Researcher open question; reframed against the no-SDK direct-fetch path shipped in CB-2.1)
  ...
  - **Mitigation (required):** **RESOLVED 2026-06-08 by CB-2.5 build (PR #41).** CB-2.5's `lib/coinbase/trace.ts` ships an `extractRateLimit` helper that reads `x-ratelimit-*` headers from every Coinbase response and embeds the parsed `{remaining, limit, reset}` shape in the structured-JSON trace log line. CB-2.5's live integration test against real Coinbase confirmed **auth'd brokerage endpoints DO return the headers** (30 RPS global limit per the observed `x-ratelimit-limit: "30, 30;w=1;name=\"global_limit\""`); **public market endpoints do NOT return them**. Active 25%-utilization warning emission is deferred (Sentry SDK install not in CB-2.5 scope per operator); the captured data lives in the trace log line and is queryable downstream. **No 25%-warning logic needed for normal operation** — the operator's ~96 ticks/day cron rate (~0.001 RPS) sits vastly under the 10%-of-ceiling guardrail.
  - **Reframing note (2026-06-06):** original wording framed this risk against an SDK that might "abstract the headers away." Now that CB-2.1's no-SDK direct-fetch is the shipped pattern, the SDK-abstraction concern is moot — but the underlying brokerage-API question (does Coinbase even send the header on `/api/v3/brokerage/*`?) was answered 2026-06-08 (yes, on auth'd; no, on public).
...
- [2026-06-06] [PM] **Three Researcher Open Questions logged above (rate-limit headers; SDK comparison freshness; CDP JWT rotation behavior)** — Researcher fills before CB-2.1 starts (**FULLY RESOLVED 2026-06-08**)
  - **Severity (required, mandatory):** P3 (informational; doesn't block brief approval)
  - **Owner (required, mandatory):** Researcher
  - **Status:** **ALL THREE ITEMS CLOSED.** Item 2 (SDK comparison) closed 2026-06-06 by CB-2.1 Engineer DRI Decision (no SDK; comparison moot). Items 1 (rate-limit headers) + 3 (CDP JWT rotation behavior of the direct path) closed 2026-06-08 by CB-2.5 build (PR #41).
  - **Area (required, tag):** research / dependency
  - **Resolution (filled when closed):** Item 2 — see CB-2.1 Engineer DRI Decision (no SDK; direct REST + JWT). Item 1 — CB-2.5 empirical live-API discovery: auth'd brokerage returns `x-ratelimit-*` headers (30 RPS global limit); public market does not return them; `trace.ts:extractRateLimit` captures values when present. Item 3 — CB-2.5 code-review: `jwt.ts:mintJWT` reads `env()` per call; inter-request rotation works automatically without a process-restart. Both resolutions documented in [PR #41](https://github.com/vivekschaudhary/crypto-bot/pull/41) commit + the brief's PM Risk #2 mitigation entry above.
