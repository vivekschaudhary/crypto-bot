...
|---|---|---|---|---|---|
| [CB-1](bets/CB-1/brief.md) | CB-1.1 + CB-1.1.1 + CB-1.2 shipped; CB-1.3 (`ready`) — authentication endpoints next | PM → Engineer | `/build CB-1.3` | 2026-05-31 | 2026-06-14 (per plan.md, high confidence on CB-1) |
...
- **2026-06-01** — [CB-1.3 story](bets/CB-1/stories/CB-1.3/story.md) drafted (`status: ready`) — passkey authentication ceremony endpoints (`POST /api/auth/authenticate/{begin,finish}`). 12 ACs: route handlers + user-exists precondition + counter-replay protection (Apple-authenticator-friendly) + session rotation via `lib/auth/sessions.rotateSession` (extended additively with optional `txClient` param, mirroring CB-1.2's `createSession` extension) + Codex E2E (AC 8). 3 PM DRI Decisions + 1 Engineer-anticipated Decision + 3 Risks. Multi-device authentication deferred per portfolio. Next: `/build CB-1.3`.
...
- **`/plan` v3 refreshed 2026-06-01.** CB-1.2's story.md creation fires the "Stories created" trigger per the estimate model. Per the new [adaptive-decomposition resolution rule](foundation/plan.md#decisions), `duration_weeks = max(3 × 3 days, 2 wk) = 2 wk` — net date movement zero, but the trigger fire is now logged in the refinement log with the triggering artifact path. **Lesson:** prior PR #6 draft claimed `/plan` could be skipped under strict-derivation — Codex review correctly flagged that as overstating the rule; the trigger fires on each new story.md, even when the resulting math collapses back to the brief-approval value.
