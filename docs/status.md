...
- **2026-06-01** — [CB-1.4 story](bets/CB-1/stories/CB-1.4/story.md) drafted (`status: ready`) — real session validation in `app/proxy.ts` replacing the scaffold-stub TODO. 12 ACs covering: gate logic + branch on route class (302 to landing with `?next` for dashboard, 401 JSON for API) + `x-session-user-id` / `x-session-id` header enrichment for downstream handlers + Node runtime declaration + Vitest tests + Codex E2E (AC 8, third in the codebase). 3 PM DRI Decisions (interim redirect target, header context-passing, Node runtime); 3 Risks (DB-roundtrip latency, sliding-expiry write amplification, proxy-coverage cascade). Closes CB-1 guardrail #1 (zero unauthenticated capital-touching requests) from runtime-enforcement standpoint. Next: `/build CB-1.4`.
